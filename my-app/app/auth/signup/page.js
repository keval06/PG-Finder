...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[0-9]{10}$/.test(mobile)) { setError("Mobile number must be exactly 10 digits"); return; }
    ...
    try {
      const res  = await fetch("http://localhost:5000/api/user/signup", {
        ...
        body: JSON.stringify({ name: name.trim(), mobile: mobile.trim(), password }),
      });
      ...
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col gap-5">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            {password.length > 0 && password.length < 8 && (
              <p className="text-xs text-red-500 px-1 flex items-center gap-1">
                <XCircle size={13} /> At least 8 characters ({password.length}/8)
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            {showMatchIndicator && (
              <p className={`text-xs px-1 flex items-center gap-1 ${passwordsMatch ? "text-green-500" : "text-red-500"}`}>
                {passwordsMatch ? <><CheckCircle2 size={13} />Passwords match</> : <><XCircle size={13} />Passwords do not match</>}
              </p>
            )}
          </div>
