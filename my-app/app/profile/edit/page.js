...
export default function EditProfilePage() {
  ...
  const confirmUpdate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: "error", text: data.message || "Update failed. Please try again." });
      }
    } catch {
  ...
  const inputClass = "w-full bg-transparent pl-9 pr-10 py-2.5 text-sm outline-none placeholder:text-slate-400 text-slate-900";
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="max-w-sm mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col gap-5">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* Mobile */}
            <div>
              <label className="text-xs font-medium text-slate-500 mb-1.5 block">Mobile Number</label>
              <div className="relative flex items-center border border-slate-200 rounded-xl bg-slate-50 focus-within:bg-white focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
            ...
            <div className="border-t border-slate-100 pt-2">
              <p className="text-xs text-slate-400 mb-3">Leave password fields empty to keep current password</p>
              ...
              <div className="flex flex-col gap-1">
                ...
                {showMatchIndicator && (
                  <p className={`text-xs px-1 flex items-center gap-1 ${passwordsMatch ? "text-green-600" : "text-red-500"}`}>
                    {passwordsMatch
            ...
            <button
              type="submit"
              disabled={loading || name.trim().length < 3 || passwordTooShort || (showMatchIndicator && !passwordsMatch)}
      ...
      {showConfirmPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setShowConfirmPopup(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xs flex flex-col gap-4 z-10 border border-slate-100">
            <div className="text-center">
              ...
              <h3 className="text-base font-bold text-slate-900">Save Changes?</h3>
              <p className="text-sm text-slate-400 mt-1">Are you sure you want to update your profile?</p>
            </div>

            <div className="bg-slate-50 rounded-xl p-3 flex flex-col gap-1.5 text-xs text-slate-600 border border-slate-100">
              {pendingBody?.name     && <p>• Name → <span className="font-semibold text-slate-900">{pendingBody.name}</span></p>}
              {pendingBody?.mobile   && <p>• Mobile → <span className="font-semibold text-slate-900">{pendingBody.mobile}</span></p>}
