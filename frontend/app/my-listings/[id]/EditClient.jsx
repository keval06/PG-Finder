...
export default function EditListingClient({ pgId }) {
  ...
  const bathroomRatio = pg.bathroom && pg.room ? `${pg.bathroom} (${pg.bathroom >= pg.room ? "Attached" : `1:${Math.round(pg.room / pg.bathroom)}`})` : "—";
  const toiletRatio = pg.toilet && pg.room ? `${pg.toilet} (${pg.toilet >= pg.room ? "Attached" : `1:${Math.round(pg.room / pg.toilet)}`})` : "—";
  const totalBeds = roomTypes.reduce((s, rt) => s + rt.availableRooms * rt.sharingCount, 0);
  const freeBeds = roomTypes.reduce((s, rt) => s + (rt.remainingBeds ?? rt.availableRooms * rt.sharingCount - (rt.occupiedBeds || 0)), 0);
  ...
  return (
    <div className="bg-white min-h-screen text-[#222222] font-sans selection:bg-rose-100">
      <div className="max-w-[1360px] mx-auto px-6 md:px-12 lg:px-12 py-6">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {inactive && (
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-full border border-slate-200">
              ...
              <span className="text-xs font-semibold text-slate-600">Hidden from Public Search</span>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
               ...
               >
                 <Pencil size={14} /> {editOpen ? "Close Editor" : "Edit Listing"}
               </button>
            </div>
          </div>
        </div>
        ...
        {toast && (
          <div className={`fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-2 px-6 py-3 rounded-full text-sm font-semibold shadow-xl border animate-in fade-in slide-in-from-bottom-5 duration-300 ${
            toast.type === "success" ? "bg-[#222222] text-white border-transparent" : "bg-red-600 text-white border-transparent"
          }`}>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-7">
            ...
            {editOpen && (
              <div className="mb-10 bg-white border-2 border-rose-400 rounded-3xl p-8 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-xl font-bold text-[#222222]">Editor Mode</h2>
                    <p className="text-sm text-gray-500 mt-0.5">Update listing details and room types.</p>
                  </div>
            ...
            <div className="pb-6 border-b border-[#DDDDDD]">
              <h2 className="text-[22px] font-semibold text-[#222222]">Management Dashboard</h2>
              <p className="text-[#484848] text-base mt-1">
                {pg.room} rooms · {totalBeds} total beds · {freeBeds} currently free
              </p>
            </div>
            ...
            <div className="py-8 border-b border-[#DDDDDD] space-y-6">
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h3 className="text-[15px] font-semibold text-[#222222]">Property Status</h3>
                  <p className="text-[#717171] text-sm mt-0.5 leading-relaxed">
                    This listing is currently {pg.isActive ? <span className="text-emerald-600 font-bold">Active</span> : <span className="text-rose-600 font-bold">Inactive</span>}. {pg.isActive ? "It is visible to all potential guests on the platform." : "Guests cannot find or book this property."}
                  </p>
                </div>
              </div>
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h3 className="text-[15px] font-semibold text-[#222222]">Inventory Management</h3>
                  <p className="text-[#717171] text-sm mt-0.5 leading-relaxed">
                    You have {roomTypes.length} room types defined. Keep your prices and availability updated to maintain high search rankings.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="py-8 border-b border-[#DDDDDD]">
              <h2 className="text-[22px] font-semibold mb-5 text-[#222222]">Room configurations</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {[
                  { icon: ROOM_DETAIL_ICONS.Rooms, label: "Private & Shared Rooms", value: `${pg.room} rooms` },
                  { icon: ROOM_DETAIL_ICONS.Gender, label: "Gender Specification", value: GENDER_LABELS[pg.gender] || pg.gender },
                  { icon: ROOM_DETAIL_ICONS.Food, label: "Food Service", value: FOOD_LABELS[pg.food] || pg.food },
                  { icon: ROOM_DETAIL_ICONS.Bathroom, label: "Bathroom Ratio", value: bathroomRatio },
                  { icon: ROOM_DETAIL_ICONS.Toilet, label: "Toilet Facility", value: toiletRatio },
                  { icon: ROOM_DETAIL_ICONS["Free Beds"], label: "Current Availability", value: `${freeBeds} beds free` },
                ].map(({ icon: Icon, label, value }) => (
                  <div key={label} className="p-3 border border-gray-200 rounded-xl flex items-center gap-3">
            ...
            {roomTypes.length > 0 && (
              <div className="py-8 border-b border-gray-200">
                <h2 className="text-[22px] font-semibold mb-5">Room Type Performance</h2>
                <div className="grid grid-cols-1 gap-4">
                  {roomTypes.map((rt) => {
                    ...
                    return (
                      <div key={rt._id} className="p-5 border border-gray-200 rounded-2xl bg-white shadow-sm hover:border-gray-300 transition-all">
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            <p className="text-base font-bold text-[#222222] capitalize">{rt.name}</p>
                            <p className="text-xs text-gray-500 font-medium">{rt.sharingCount}-sharing · {rt.availableRooms} rooms</p>
                          </div>
                          <div className="text-right">
                            <p className="text-lg font-bold text-rose-500">₹{rt.price?.toLocaleString("en-IN")}</p>
                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Per Bed</p>
                          </div>
                        </div>
                        <div className="mt-4">
                          <div className="flex items-center justify-between text-xs mb-1.5 font-semibold">
                            <span className="text-gray-600">Occupancy Level</span>
                            <span className={free > 0 ? "text-emerald-600" : "text-rose-600"}>{free} beds available</span>
                          </div>
            ...
            {/* Amenities Section */}
            <div className="py-8 border-b border-[#DDDDDD]">
              <h2 className="text-[22px] font-semibold mb-5 text-[#222222]">What this place offers</h2>
                ...
                {(pg.amenities || []).map((item) => {
                  ...
                  return (
                    <div key={item} className="flex items-center gap-4 text-gray-700">
                      ...
                      <span className="text-[15px] text-[#484848] font-normal">{item}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="lg:col-span-5 relative">
            <div className="lg:sticky lg:top-28">
              <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-[0_6px_16px_rgba(0,0,0,0.12)]">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Owner Controls</span>
                ...
                <div className="border border-gray-200 rounded-xl p-4 bg-gray-50/50 mb-6">
                   <p className="text-[10px] font-bold uppercase text-gray-400 mb-3 tracking-widest">Occupancy Stats</p>
                   <div className="space-y-4">
                      <div className="flex justify-between items-center">
                         <span className="text-sm font-medium text-gray-600">Total Capacity</span>
                         <span className="text-sm font-bold">{totalBeds} beds</span>
                      </div>
                      <div className="flex justify-between items-center">
                         <span className="text-sm font-medium text-gray-600">Current Vacancy</span>
                         <span className={`text-sm font-bold ${freeBeds > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{freeBeds} beds</span>
                      </div>
                      <div className="flex justify-between items-center">
                         <span className="text-sm font-medium text-gray-600">Allocated Rooms</span>
                         <span className="text-sm font-bold">{pg.allocatedRooms || 0} / {pg.room}</span>
                      </div>
                   </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-100 flex items-center gap-3 text-xs text-gray-400 italic leading-snug">
                   ...
                   <span>Only you can see this dashboard. Guests see the premium booking interface.</span>
                </div>
              </div>
              ...
              <div className="mt-6 p-5 border border-gray-200 rounded-xl bg-gray-50/50">
                ...
                <p className="text-xs text-gray-500 leading-relaxed">
                  Listing updated on {new Date(pg.updatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}.
                  Keep your availability and pricing current to improve conversion.
                </p>
              </div>
            </div>
          </div>
      ...
      <ConfirmModal
        ...
        description={inactive ? "This will make your PG visible to all students." : "Students will no longer be able to find or book this PG."}
      ...
      <ConfirmModal
        ...
        onConfirm={() => { rtDeleteTarget.doDelete(); setRtDeleteTarget(null); }}
