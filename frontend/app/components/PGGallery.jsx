      ...
      <div>
        <div className="w-full h-[420px] bg-gray-200 rounded-2xl flex items-center justify-center">
          <p className="text-gray-500 capitalize">
            No {activeCategory} images available
          </p>
        </div>
        ...
        <div className="flex gap-3 mt-3 overflow-x-auto pb-1 scrollbar-hide">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              onClick={() => { setActiveCategory(cat); setActiveImage(0); }}
              className={`px-4 py-1 rounded-full capitalize text-sm border flex-shrink-0 ${
                activeCategory === cat
      ...
      <div className="flex gap-3 mt-3 overflow-x-auto pb-1 scrollbar-hide">
        {filtered.map((img, i) => (
          <img
            ...
            alt={img.category}
            className={`w-24 h-16 object-cover rounded cursor-pointer border-2 flex-shrink-0 ${
