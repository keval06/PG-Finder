    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const [dbRes, nominatimRes] = await Promise.allSettled([
          fetch(
            `${API}/api/pg/suggestions?q=${encodeURIComponent(query)}`
          ).then((r) => (r.ok ? r.json() : { data: [] })),
          fetch(
            `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(
              query
            )}&format=json&addressdetails=1&countrycodes=in&limit=4&dedupe=1&accept-language=en`
          ).then((r) => r.json()),
        ]);

        const results = [];

        // DB results → extract cities and their accurate PG count
        if (dbRes.status === "fulfilled" && dbRes.value?.data?.length) {
          for (const item of dbRes.value.data) {
            ...
            if (city) {
              results.push({
                _source: "db",
                name: city,
                display_name: `${city} — ${count} PG${count !== 1 ? 's' : ''} found`,
                address: { city, state: "" },
              });
            }
          }
        ...
        if (nominatimRes.status === "fulfilled" && nominatimRes.value?.length) {
          const unique = Array.from(
            new Map(
              nominatimRes.value.map((item) => [item.display_name, item])
            ).values()
          );
          ...
          for (const item of unique) {
            const city =
              ...
              item.name;
            const alreadyHave = results.some(
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setShowSuggestions(false);
                            setSuggestions([]);
                            // Only redirect to /home if not already on a browse page
                            if (pathname !== "/home" && pathname !== "/my-listings") {
                        ...
                        >
                          ...
                          <div className="flex flex-col overflow-hidden">
                            <span className="text-[15px] font-semibold text-[#222222] truncate">
                              {city}
                            </span>
                            <span className="text-sm text-[#717171] truncate">
                              {state ? `${state}, India` : s.display_name}
                            </span>
                          </div>
                        </button>
                      );
